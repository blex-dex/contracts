...
async function initCoreVault({ coreVault, asset, name, symbol, vaultRouterAddr, feeRouterAddr, vaultRewardAddr }) {
  if (null == coreVault) coreVault = await readCoreVaultContract();
  await handleTx(
    coreVault.initialize(asset, name, symbol, vaultRouterAddr, feeRouterAddr, vaultRewardAddr),

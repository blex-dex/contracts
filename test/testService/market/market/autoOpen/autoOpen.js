...
describe("auto", async () => {
  ...
  it("Hang 10 triggers, and the test can still automatically open positions successfully", async () => {
    logger.info(
      "---Hang 10 triggers, and the test can still automatically open positions successfully---"
    );
    ...
    //increase position
    {
      ...
      //increase position
      let lastTx = await connectIncreasePosition(owner, incParams);
      ...
      //Obtain all vault USD balances after the transaction is completed for later calculations
      let afterUSD = await getUSDBalance();
      //Obtain all market PNL users and calculate later after the transaction is completed
      ...
      //The calculated AUM is compared with the subsequent AUM
    ...
    // //Open 10 Trigger orders
    {
      for (let index = 0; index < 10; index++) {
        ...
        //Calculate the collateral  for this time to reduce the position
        let collateralDelta = getDecreaseDeltaCollateral({
          size: position.size,
          dsize: inputs._order.size,
        ...
        let totalFee = totalFees(fees);
        //Get the order number of this transaction
        ...
        //Build the list of parameters needed to trigger the UpdateOrder event
        let eventArg = [
          ...
          collateralDelta, //collateral collateral  for this reduction
          inputs._order.price, //
        ...
        //Construct various verification parameters for order creation
        let vaildOrderArgs = [
          owner.address, //account
          orderID, //orderID,
        ...
        //Get the data written into the contract by order
        let orderInfo = await getOrderInfo({
          user: owner.address,
          orderID,
          label: "1",
        });
        //Compare the parameters expected to be written with the data actually written to the contract
        vaildOrderCreated({
          order: orderInfo,
          args: vaildOrderArgs,
        });
    ...
    //Get AUM from the contract
    let beforeAum = await getAUM();
    let beforeUSD = await getUSDBalance();
    let beforePNL = await getGlobalPnl();
    //Calculate AUM locally
    let beforeCalcAUM = calcAUM(beforePNL, beforeUSD);
    ...
    //execute order
    {
      ...
      //Because the user's funds have already entered the market when the order is placed, the market fund change is equal to the feeVault paid by the market
      validBalanceChange(
        totalFee,

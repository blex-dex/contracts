...
describe("1800 open long, the fund fee plus PNL together meet the liquidation standard, the test should be transferred to the vault first, and then transferred to the feeVault if there are remaining funds", async () => {
  let allContracts;
  ...
  it("1800 open long, the fund fee plus PNL together meet the liquidation standard, the test should be transferred to the vault first, and then transferred to the feeVault if there are remaining funds", async () => {
    await execPrePare({
      user: owner,
      amount: 20,
    ...
    await setPrice(allContracts.indexToken, 1750);
    ...
    //Calculate the funding fee locally
    {
      ...
      let amount = position.collateral.add(pnl).gt(0)
        ? result.pnl
        : position.collateral;
      ...
      await validTransfer({
        ...
        amount: amount,
        label: "check  the amount transferred from market to vault",
        expandDecimals: false,
      });

      //Transfer all deposits to feeVault
      await validTransfer({
        tokenContract: allContracts.USDC,
        blockNumber: lastTx.blockNumber,
        from: allContracts.market.address,
        to: allContracts.allFee.feeVault.address,
        amount: position.collateral.sub(amount),
        label: "check  the amount transferred from market to feeVault",
        expandDecimals: false,
      });
    }

...
describe("auto", async () => {
  ...
  it("(_params.isLiquidate || isCloseAll) && _params.dPnl + _params.collateral - _params.fees < 0", async () => {
    logger.info(
      "---(_params.isLiquidate || isCloseAll) && _params.dPnl + _params.collateral - _params.fees < 0---"
    );
    ...
    await setPrice(allContracts.indexToken, 44000);
    ...
    //execute order
    {
      ...
      let liqFee = numberToBigNumber(0);
      ...
      await validTransfer({
        ...
        from: allContracts.market.address,
        to: allContracts.allFee.feeVault.address,
        amount: position.collateral.sub(result.pnl),
        expandDecimals: false,
      });

...
describe("auto", async () => {
  ...
  it("Liq All Position return 1", async () => {
    logger.info("---Test Liq All Position return 1 ---");
    ...
    await setPrice(allContracts.indexToken, 44350);
    let afterAum = await getAUM();
    let afterUSD = await getUSDBalance();
    let afterPNL = await getGlobalPnl();

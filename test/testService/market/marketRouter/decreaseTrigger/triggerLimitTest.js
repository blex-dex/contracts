...
describe("Test limit order trigger limit - up to 10 triggers", async () => {
  ...
  it("After a 10trgger, the market order can still be placed", async () => {
    logger.info("---After a 10trgger, the market order can still be placed---");
...

    {
      for (let index = 0; index < 10; index++) {
        ...
        let collateralDelta = position.collateral.div(250);
    ...
    {
      //mint
      await userMintAndApprove(owner, 500, allContracts.marketRouter);
      ...
      incParams._oraclePrice = oraclePrice;
      ...
      let totalColl = incParams.collateralDelta.mul(2);
      let totalFee = totalfee.mul(2);

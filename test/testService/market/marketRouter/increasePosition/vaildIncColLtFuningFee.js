...
describe("increasePosition", async () => {
  ...
  it("Increase position size, add funding, increase margin and check collateral and funding, but the increased margin is not enough to cover the test case of funding rate calculation", async () => {
    logger.info(
      "---Test cases for increasing position size, adding funding fee, and increasing margin less than funding fee---"
    );
    ...
    //Open Position
    {
      ...
      let collD = params.collateralDelta.sub(openFee);
      //calculate pnl
      let result = calcPNL({
        position,
        price: 1700,
